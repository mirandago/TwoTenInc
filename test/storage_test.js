...
describe('Testing API for storage.js', () => {
  describe('test addGroup', () => {  
    ...
    it('Run addGroup', async () => {     
      ...
      chai.expect(JSON.parse(window.localStorage.getItem('this is mocha test'))).to.equal([]);
    });   
  ...
  describe('test addTask', () => {      
    ...
    it('test addTask', async () => {   
      ...
      chai.expect(window.localStorage.getItem('green')[0].name).to.equal(task1.name);
      chai.expect(window.localStorage.getItem('green')[0].group).to.equal(task1.group);
      chai.expect(window.localStorage.getItem('green')[0].session).to.equal(task1.session);
      chai.expect(window.localStorage.getItem('green')[0].sessionCompleted).to.equal(0);
      chai.expect(window.localStorage.getItem('green')[0].date).to.equal(task1.date);
      chai.expect(window.localStorage.getItem('green')[0].completed).to.equal(false);
    });
    ...
    it('test addTask for different task', async () => {   
      ...
      chai.expect(window.localStorage.getItem('green')[0].name).to.equal(task2.name);
      chai.expect(window.localStorage.getItem('green')[0].group).to.equal(task2.group);
      chai.expect(window.localStorage.getItem('green')[0].session).to.equal(task2.session);
      chai.expect(window.localStorage.getItem('green')[0].sessionCompleted).to.equal(0);
      chai.expect(window.localStorage.getItem('green')[0].date).to.equal(task2.date);
      chai.expect(window.localStorage.getItem('green')[0].completed).to.equal(false);
    });
    ...
    it('test getAllTasks when two groups',  async() => {
      ...
      return getAllTasks()
        .then((value) => {
          ...
          chai.expect(value).to.include.members(['task0', 'task1', 'task2', 'task3']);
        });
    });
  });
  ...
  describe('test deleteTask', () => {  
    ...
    it('test deleteTask when no task', () => {
      ...
      chai.expect(JSON.parse(window.localStorage.getItem('yellow')).length).to.equal(1);
      chai.expect(JSON.parse(window.localStorage.getItem('yellow'))[0].name).to.equal('a');
    });
   
    it('test deleteTask when only task', () => {
      deleteTask('a', 'yellow') 
      chai.expect(JSON.parse(window.localStorage.getItem('yellow')).length).to.equal(0);
    });
    
    it('test deleteTask when multiple tasks', () => {
      deleteTask('c', 'blue') 
      chai.expect(JSON.parse(window.localStorage.getItem('blue')).length).to.equal(1);
      chai.expect(JSON.parse(window.localStorage.getItem('blue'))[0].name).to.equal('b');
    });
  });
  ...
  describe('test completeTask', () => {  
    ...
    it('test completeTask when no task', () => {
      ...
      chai.expect(JSON.parse(window.localStorage.getItem('yellow'))[0].completed).to.equal(false);
    });
   
    it('test completeTask when only task', () => {
      completeTask('a', 'yellow') 
      chai.expect(JSON.parse(window.localStorage.getItem('yellow'))[0].completed).to.equal(true);
    });
    
    it('test completeTask when multiple tasks', () => {
      completeTask('b', 'blue') 
      chai.expect(JSON.parse(window.localStorage.getItem('blue'))[0].completed).to.equal(true);
      chai.expect(JSON.parse(window.localStorage.getItem('blue'))[1].completed).to.equal(false);
    });
  });
  ...
  describe('test completeSession', () => {  
    ...
    it('test completeSession when no task', () => {      
      ...
      chai.expect(JSON.parse(window.localStorage.getItem('yellow'))[0].sessionCompleted).to.equal(0);
    });
   
    it('test completeSession when only task', () => {      
      completeSession('a', 'yellow') 
      chai.expect(JSON.parse(window.localStorage.getItem('yellow'))[0].sessionCompleted).to.equal(1);
      completeSession('a', 'yellow') 
      chai.expect(JSON.parse(window.localStorage.getItem('yellow'))[0].sessionCompleted).to.equal(2);
    });
    
    it('test completeSession when multiple tasks', () => {
      completeSession('b', 'blue') 
      chai.expect(JSON.parse(window.localStorage.getItem('blue'))[0].sessionCompleted).to.equal(1);
      chai.expect(JSON.parse(window.localStorage.getItem('blue'))[1].sessionCompleted).to.equal(0);      
      completeSession('c', 'blue') 
      chai.expect(JSON.parse(window.localStorage.getItem('blue'))[0].sessionCompleted).to.equal(1);
      chai.expect(JSON.parse(window.localStorage.getItem('blue'))[1].sessionCompleted).to.equal(1);
      completeSession('b', 'blue') 
      chai.expect(JSON.parse(window.localStorage.getItem('blue'))[0].sessionCompleted).to.equal(2);
      chai.expect(JSON.parse(window.localStorage.getItem('blue'))[1].sessionCompleted).to.equal(1);
    });
  });
});
